...
import torch
from models import GlyphGenerator, TextureGenerator, Zencoder
from utils import load_image, to_data, to_var, visualize, save_image, gaussian,label_input, load_style_image_pair
import os
...
def main():
    ...
    style_codem =  myZencoder(input=Y, segmap=labels)
    ...
    if label == -1:
        ...
        result = []
        while scale <= 1.0: 
            img_str = netGlyph(text, scale,labels,style_codem) 
            img_str[:,0:1] = torch.clamp(img_str[:,0:1] + noise, -1, 1)
            result1 = netTexture(img_str,labels,style_codem).detach()
