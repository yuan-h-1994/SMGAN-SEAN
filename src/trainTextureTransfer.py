...
def main():
    ...
    netShapeM = ShapeMatchingGAN(opts.GS_nlayers, opts.DS_nlayers, opts.GS_nf, opts.DS_nf,
                     opts.GT_nlayers, opts.DT_nlayers, opts.GT_nf, opts.DT_nf,
                     opts.DG_nlayers, opts.DG_nf, opts.GG_nlayers, opts.GG_nf, opts.gpu)
    if opts.gpu:
        netShapeM.cuda()
    ...
    for epoch in range(opts.texture_step1_epochs):
        for i in range(opts.Ttraining_num//opts.batchsize):
            for j in range(opts.style_number):
                ...
                labels = label_input(label_name,opts.batchsize//8, opts.gpu)
                labels = to_var(labels) if opts.gpu else labels
                _, X, Y, Noise = load_style_image_pair(style_name, gpu=opts.gpu)
                Y = to_var(Y) if opts.gpu else Y
                X = to_var(X) if opts.gpu else X
                Noise = to_var(Noise) if opts.gpu else Noise
                x, y = cropping_training_batches(X, Y, Noise, opts.batchsize//8, 
                                        opts.Tanglejitter, opts.subimg_size, opts.subimg_size)
                style_code =  myZencoder(input=y, segmap=labels)
    ...
    if opts.style_loss:
        ...
        for epoch in range(opts.texture_step2_epochs):
            ...
            for fname in fnames:
                ...
                losses = netShapeM.texture_one_pass(x, y, labels, t, 0, VGGfeatures, style_targets)  
